...
class FlextSingerArchitectureStandardizer:
    ...
    def standardize_architecture(self) -> dict[str, Any]:
        ...
        for project in all_projects:
            ...
            if project_changes > 0:
                results["projects_standardized"].append(project.name)
                results["changes_made"] += project_changes
            else:
                pass

        results["files_processed"] = self._count_python_files(all_projects)
        ...
        if "from flext_meltano" in content and "# MIGRATED:" not in content:
            ...
            for i, line in enumerate(lines):
                if line.startswith("from flext_meltano"):
                    lines.insert(i, "# MIGRATED: Singer SDK imports centralized via flext-meltano")
                    break
    ...
    def _count_python_files(self, projects: list[Path]) -> int:
