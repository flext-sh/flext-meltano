...
if result.success:
    print(f"Pipeline processed {result.value.records_processed} records")
```
...
def bridge_execute_unified_pipeline(config_json: str) -> dict[str, object]:
    ...
    return {
        ...
        "records_processed": result.value.records_processed if result.success else 0,
        "execution_time_ms": result.value.execution_time_ms if result.success else 0,
    ...
    def validate_business_rules(self) -> FlextResult[None]:
        ...
        if not self.name or not isinstance(self.name, str):
            return FlextResult[None].fail(
                "Singer plugin name must be a non-empty string"
            )

        if not self.config or not isinstance(self.config, dict):
            return FlextResult[None].fail(
                "Singer config must be a non-empty dictionary"
            )
...
class FlextSingerUnifiedService(FlextDomainService[FlextSingerUnifiedResult]):
    ...
    def execute_operation(self, *args: object, **kwargs: object) -> FlextResult[object]:
        ...
        if operation == "execute_pipeline":
            ...
            return (
                FlextResult[object].ok(pipeline_result.value)
        ...
        if operation == "discover_catalogs":
            ...
            return (
                FlextResult[object].ok(catalogs_result.value)
        ...
        if operation == "validate_components":
            ...
            return (
                FlextResult[object].ok(validation_result.value)
    ...
    def _execute_pipeline_operation(
        ...
        for param in required_params:
            if param not in kwargs:
                return FlextResult[FlextSingerUnifiedResult].fail(f"Missing required parameter: {param}")
        ...
        try:
            ...
            return FlextResult[FlextSingerUnifiedResult].fail(f"Invalid pipeline configuration: {e}")
    ...
    def register_component(
        ...
        try:
            ...
            return FlextResult[None].fail(f"Failed to register component '{name}': {e}")
    ...
    def get_component(self, name: str) -> FlextResult[FlextSingerUnifiedInterface]:
        ...
        try:
            if name not in self._registered_components:
                return FlextResult[FlextSingerUnifiedInterface].fail(f"Component '{name}' is not registered")

            return FlextResult[FlextSingerUnifiedInterface].ok(self._registered_components[name])

        except (KeyError, ValueError) as e:
            return FlextResult[FlextSingerUnifiedInterface].fail(f"Failed to get component '{name}': {e}")
    ...
    def execute_pipeline(
        ...
        try:
            ...
            if components_result.is_failure:
                return FlextResult[FlextSingerUnifiedResult].fail(
                    components_result.error or "Component initialization failed",
                )

            # Type-safe extraction of components tuple
            components_data = components_result.value
            if components_data is None:
                return FlextResult[FlextSingerUnifiedResult].fail("Components result data is None")
            ...
            return FlextResult[FlextSingerUnifiedResult].fail(f"Pipeline execution failed: {e}")
    ...
    def _get_and_initialize_components(
        ...
        components_data = components_result.value
        if components_data is None:
            return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].fail("Components result data is None")
        ...
        if initialization_result.is_failure:
            return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].fail(
                initialization_result.error or "Component initialization failed",
            )

        return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].ok((tap, target))
    ...
    def _get_components(
        ...
        if tap_result.is_failure:
            return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].fail(f"Tap error: {tap_result.error}")

        target_result = self.get_component(pipeline_config.target_name)
        if target_result.is_failure:
            return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].fail(f"Target error: {target_result.error}")
        ...
        if tap is None:
            return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].fail("Tap component is None")
        if target is None:
            return FlextResult[tuple[FlextSingerUnifiedInterface, FlextSingerUnifiedInterface]].fail("Target component is None")
    ...
    def _execute_pipeline_steps(
        ...
        if extract_result.is_failure:
            return FlextResult[FlextSingerUnifiedResult].fail(f"Extract failed: {extract_result.error}")

        # Type-safe extraction of extract data
        extract_data = extract_result.value
        if extract_data is None:
            return FlextResult[FlextSingerUnifiedResult].fail("Extract result data is None")

        # Load data to target
        load_result = target.execute(extract_data)
        if load_result.is_failure:
            return FlextResult[FlextSingerUnifiedResult].fail(f"Load failed: {load_result.error}")

        # Type-safe extraction of load data
        load_data = load_result.value
        if load_data is None:
            return FlextResult[FlextSingerUnifiedResult].fail("Load result data is None")
        ...
        return FlextResult[FlextSingerUnifiedResult].ok(combined_result)
    ...
    def discover_all_catalogs(self) -> FlextResult[dict[str, dict[str, object]]]:
        ...
        try:
            ...
            for name, component in self._registered_components.items():
                ...
                if catalog_result.success and catalog_result.value is not None:
                    catalogs[name] = catalog_result.value

            return FlextResult[dict[str, dict[str, object]]].ok(catalogs)

        except (ValueError, TypeError, RuntimeError) as e:
            return FlextResult[dict[str, dict[str, object]]].fail(f"Catalog discovery failed: {e}")
    ...
    def validate_all_components(self) -> FlextResult[dict[str, bool]]:
        ...
        try:
            ...
            return FlextResult[dict[str, bool]].fail(f"Component validation failed: {e}")
