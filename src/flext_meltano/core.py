...
try:
    ...
    # Fallback if singer_sdk.typing is not available
    try:
        from singer_sdk import typing as singer_typing
        PropertiesList = getattr(singer_typing, "PropertiesList", list)
...
class FlextMeltanoRepository(Repository[FlextMeltanoPipelineResult]):
    ...
    async def save(self, result: FlextMeltanoPipelineResult) -> FlextResult[str]:
        ...
        try:
            ...
            return FlextResult(data=result.pipeline_id)
        except (OSError, KeyError, ValueError, TypeError) as e:
            return FlextResult(error=f"Failed to save pipeline result: {e}")
    ...
    async def get_by_id(self, pipeline_id: str) -> FlextResult[FlextMeltanoPipelineResult]:
        ...
        try:
            ...
            return FlextResult(error=f"Pipeline result not found: {pipeline_id}")
        except (KeyError, ValueError) as e:
            return FlextResult(error=f"Failed to retrieve pipeline result: {e}")
    ...
    async def get_all(self) -> FlextResult[list[FlextMeltanoPipelineResult]]:
        ...
        try:
            ...
            return FlextResult(error=f"Failed to retrieve pipeline results: {e}")
    ...
    async def delete_by_id(self, pipeline_id: str) -> FlextResult[bool]:
        ...
        try:
            if pipeline_id in self._storage:
                ...
                return FlextResult(data=deletion_successful)
            return FlextResult(error=f"Pipeline result not found: {pipeline_id}")
        except (OSError, KeyError) as e:
            return FlextResult(error=f"Failed to delete pipeline result: {e}")
    ...
    async def get_by_state(self, state: FlextMeltanoExecutionState) -> FlextResult[list[FlextMeltanoPipelineResult]]:
        ...
        try:
            ...
            return FlextResult(error=f"Failed to filter pipeline results: {e}")
    ...
    async def clear_all(self) -> FlextResult[int]:
        ...
        try:
            ...
            self._storage.clear()
            self._save_to_disk()
            return FlextResult(data=count)
        except OSError as e:
            return FlextResult(error=f"Failed to clear pipeline results: {e}")
...
class FlextMeltanoSingerService(FlextDomainService):
    ...
    def execute(self) -> FlextResult[dict[str, Any]]:
        ...
        return FlextResult(data={"service": "singer", "status": "initialized"})
    ...
    async def discover_catalog(
        ...
        try:
            ...
            if not catalog:
                return FlextResult(error=f"No catalog discovered for tap {tap_instance.name}")
    ...
    async def test_connection(self, tap_instance: Tap) -> FlextResult[bool]:
        ...
        try:
            ...
            return FlextResult(data=catalog_result.is_success)
    ...
    def get_stream_schemas(self, tap_name: str) -> FlextResult[PropertiesList]:
        ...
        if tap_name not in self._discovered_catalogs:
            return FlextResult(error=f"No catalog found for tap: {tap_name}")
        ...
        return FlextResult(data=schemas)
...
class FlextMeltanoDbtService:
    ...
    def execute(self) -> FlextResult[dict[str, Any]]:
        ...
        return FlextResult(data={"service": "dbt", "project_dir": str(self.project_dir)})
        ...
        try:
            ...
            try:
                ...
                if result.success:
                    ...
                    return FlextResult(data=run_results)
        ...
        try:
            ...
            try:
                ...
                if result.success:
                    ...
                    return FlextResult(data=test_results)
    ...
    async def execute_pipeline(
        ...
        try:
            ...
            if not connection_result.is_success:
                ...
                await self.repository.save(result)
                return FlextResult(error=connection_result.error)
