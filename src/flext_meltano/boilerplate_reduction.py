...
from flext_meltano.core import FlextMeltanoExecutionState, FlextMeltanoPipelineResult
from flext_meltano.helpers.execution import (
    flext_meltano_execute_job,
)


# Simple functions to make boilerplate reduction work
def flext_meltano_pipeline_sync(
    tap: str,
    target: str,
    **config: object,
) -> FlextMeltanoPipelineResult:
    """Simple sync pipeline execution."""
    result = flext_meltano_execute_job(tap, target, ".", "dev")
    return FlextMeltanoPipelineResult(
        pipeline_id=f"sync-{tap}-{target}",
        state=FlextMeltanoExecutionState.SUCCESS
        if result.success
        else FlextMeltanoExecutionState.FAILED,
...
async def flext_meltano_pipeline(
    ...
    **config: object,
    ...
    """Simple async pipeline execution."""
    return flext_meltano_pipeline_sync(tap, target, **config)
...
class QuickConfig:
    ...
    POSTGRES_TAP: ClassVar[dict[str, Any]] = {"tap": "tap-postgres", "schema": "public"}
    CSV_TAP: ClassVar[dict[str, Any]] = {"tap": "tap-csv", "files": []}
    ORACLE_TAP: ClassVar[dict[str, Any]] = {"tap": "tap-oracle", "sqlnet": True}

    JSON_TARGET: ClassVar[dict[str, Any]] = {
        "target": "target-jsonl",
        "path": "output/",
    }
    CSV_TARGET: ClassVar[dict[str, Any]] = {"target": "target-csv", "path": "output/"}
    POSTGRES_TARGET: ClassVar[dict[str, Any]] = {
        ...
        def wrapper(*args: object, **kwargs: object) -> object:
            ...
            for attempt in range(max_retries + 1):
                ...
                if attempt < max_retries:
                    time.sleep(delay * (BACKOFF_BASE**attempt))  # Exponential backoff
