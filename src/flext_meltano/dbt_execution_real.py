...
class DbtRealExecutor(FlextDomainService[None]):
    ...
    def create_test_dbt_project(self, project_name: str = "test_dbt") -> FlextResult[Path]:
    ...
    def run_dbt_command(self, project_path: Path, command: list[str]) -> FlextResult[dict[str, str]]:
        ...
        try:
            ...
            self.logger.info("Running DBT command", command=full_command, project=str(project_path))
            ...
            if getattr(result, 'success', False):
                ...
                self.logger.warning("DBT command failed", command=command, result=result)
    ...
    def run_dbt_run(self, project_path: Path, models: list[str] | None = None) -> FlextResult[dict[str, str]]:
    ...
    def run_dbt_test(self, project_path: Path, models: list[str] | None = None) -> FlextResult[dict[str, str]]:
...
def execute_real_dbt_workflow() -> dict[str, str]:
    ...
    if parse_result["status"] == "success":
        ...
        if compile_result["status"] == "success":
            ...
            return {"workflow": "stopped_at_compile", "parse": "success", "compile": "failed"}
    else:
