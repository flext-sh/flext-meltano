...
from flext_core.domain.entities import Pipeline, PipelineExecution, Plugin, PluginType
from flext_core.domain.pydantic_base import DomainValueObject
from flext_core.engine.meltano_wrapper import MeltanoEngine, MeltanoExecutionResult
...
class MeltanoPluginDescriptor(DomainValueObject):
    ...
    hub format and FLEXT's domain model.
    """
...
class UnifiedMeltanoAntiCorruptionLayer:
    ...
    translation between FLEXT's domain entities and Meltano's external API.
    ...
    async def translate_and_run_pipeline(
        ...
        Converts a FLEXT Pipeline entity to Meltano's execution format, handles
    ...
    async def _execute_single_step_pipeline(
        ...
        plugin_name = f"flext_{step.step_id}"
    ...
    async def _execute_two_step_pipeline(
        ...
        if extractor_step and loader_step:
            result = await self.engine.run_pipeline(
                extractor=f"flext_{extractor_step.step_id}",
                loader=f"flext_{loader_step.step_id}",
                state_id=f"flext_pipeline_{pipeline.pipeline_id.value}",
    ...
    async def _create_meltano_job_from_pipeline(
        ...
        try:
            ...
            for step in sorted_steps:
                task_name = f"flext_{step.step_id}"
                tasks.append(task_name)

            # Create job configuration
            job_config: dict[str, Any] = {
                "name": f"flext_pipeline_{pipeline.name.value}",
                "tasks": tasks,
                "description": pipeline.description
                or f"FLEXT Pipeline: {pipeline.name.value}",
            }
    ...
    async def install_plugin_from_domain(
        ...
        try:
            ...
            result = await self.engine.add_plugin(
                ...
                plugin_name=f"flext_{plugin.name}",
            )
            ...
            success_result = MeltanoExecutionResult(
                ...
                stderr="",
                command=["meltano", "add", meltano_type, f"flext_{plugin.name}"],
                execution_time=0.0,
    ...
    async def remove_plugin_from_domain(
        ...
        try:
            ...
            execution_result = MeltanoExecutionResult(
                ...
                command=["meltano", "remove", meltano_type, f"flext_{plugin.name}"],
                execution_time=0.0,
                started_at=datetime.now(UTC),
                completed_at=datetime.now(UTC),
            )
