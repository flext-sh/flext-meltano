...
class MeltanoEltExecutor(FlextDomainService[None]):
                    ...
                    "extractors": [
                        {
                            ...
                            "config": {
                                "files": [
                                    {"path": "sample.csv", "entity": "sample_data"}
                                ]
                            },
                        }
                    ],
    ...
    def install_plugins(self, project_path: Path) -> FlextResult[bool]:
        ...
        try:
            ...
            with runner.isolated_filesystem():
                ...
                self.logger.info(
                    "Installing Meltano plugins", project=str(project_path)
                )
    ...
    def test_plugin_discovery(
        self, project_path: Path, plugin_name: str
...
def execute_real_elt_pipeline() -> dict[str, int]:
    ...
    if install_success:
        ...
        ).unwrap_or({"exit_code": -1, "output_files": 0, "success": 0})
        return elt_result
    else:
