"""FLEXT Meltano Singer Adapters - Single Class Architecture (Flext[Area][Module] pattern).

**Architecture Compliance**: Single main class FlextMeltanoAdapters following Flext[Area][Module] pattern
**Hierarchical Inheritance**: Inherits from flext-core adapter patterns
**SOLID Principles**: Single Responsibility - All Meltano Singer SDK integration under one class
**ZERO Duplication**: Uses internal classes with aliases, delegates to base implementations

Enterprise Singer SDK integration with FLEXT patterns using facade pattern to eliminate duplication while maintaining compatibility.
...
class FlextMeltanoAdapters:
    """Single main adapters class for all Meltano Singer integration (Flext[Area][Module] pattern).

    Architectural Compliance:
    - All Singer SDK adapters organized under single class
    - Nested classes implement specific adapter types
    - Aliases for backward compatibility
    - Hierarchical inheritance from flext-core

    SOLID Principles:
    - Single Responsibility: All Singer SDK integration in one place
    ...
    class SingerWrapper(FlextDomainService[FlextMeltanoTypes.Singer.TapConfig]):
        ...
        def create_tap(
            self, tap_class: type[Tap], config: FlextMeltanoTypes.Singer.TapConfig
        ) -> FlextResult[Tap]:
        ...
        def run_elt_pipeline_real(
            ...
            target_config: FlextMeltanoTypes.Singer.TargetConfig,
            ...
            try:
                ...
                pipeline_metrics: FlextMeltanoTypes.ELT.PipelineResult = {
    ...
    @staticmethod
    def adapt_records(
        singer_records: Iterator[FlextMeltanoTypes.Singer.MessageData],
    ) -> Iterator[FlextMeltanoTypes.Singer.MessageData]:
    ...
    # =================================================================
    # ALIASES FOR BACKWARD COMPATIBILITY - FlextMeltano[AdapterType]
    # =================================================================

    # Main class aliases (preferred names)
    FlextMeltanoSingerWrapper = SingerWrapper
    FlextMeltanoTypeAdapter = TypeAdapter
...
MeltanoSingerWrapper = FlextMeltanoAdapters.SingerWrapper
FlextSingerAdapter = FlextMeltanoAdapters.TypeAdapter
