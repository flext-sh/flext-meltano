...
class FlextTargetPlugin(FlextPlugin, ABC):
    ...
    @abstractmethod
    def load_data(self, data: list[dict[str, Any]], config: dict[str, Any]) -> FlextResult[bool]:
...
class FlextDbtPlugin(FlextPlugin, ABC):
    ...
    @abstractmethod
    def run_models(self, project_dir: Path, models: list[str] | None = None) -> FlextResult[dict[str, Any]]:
