...
class FlextMeltanoEvent(FlextEntity):
    """Meltano event entity with complete tracking."""

    # Ensure immutability to match test expectations
    model_config = ConfigDict(frozen=True)

    id: FlextEntityId = Field(
      default_factory=lambda: FlextEntityId(str(uuid4())), description="Event ID",
    )
    event_type: str = Field(..., description="Type of event")
    timestamp: FlextTimestamp = Field(
...
class FlextMeltanoPipelineExecution(FlextEntity):
    ...
    started_at: FlextTimestamp = Field(
      default_factory=lambda: FlextTimestamp(datetime.now(UTC)),
    )
    completed_at: datetime | None = Field(default=None)
...
class FlextMeltanoPluginRegistry(FlextModel):
    ...
    last_updated: FlextTimestamp = Field(
      default_factory=lambda: FlextTimestamp(datetime.now(UTC)),
    )
...
class FlextMeltanoBridgeRequest(FlextModel):
    ...
    correlation_id: FlextEntityId = Field(
...
# =============================================================================
# CONFIGURATION (consolidated from config.py)
# =============================================================================
