...
DomainEntity = get_domain_entity()  # DomainEntity can be used as value object
...
class FlextMeltanoExecutionConfig(BaseModel):
...
class FlextMeltanoStateConfig(BaseModel):
...
class FlextMeltanoBusinessConfig(BaseModel):
...
class FlextMeltanoPluginConfig(BaseModel):
...
class FlextMeltanoMonitoringConfig(BaseModel):
...
class FlextMeltanoSettings(
    ...
    def configure_dependencies(self, container: Any | None = None) -> None:
        ...
        result = container.register("flext_meltano_settings", self)
        if hasattr(result, "is_success") and not result.is_success:
            # For real FlextContainer, handle errors, for mock containers ignore
            pass
