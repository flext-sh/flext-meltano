# FLEXT CORE MELTANO - ENTERPRISE INTEGRATION LAYER
...
The FLEXT Core Meltano module provides **enterprise-grade integration** with the Meltano SDK ecosystem, delivering:
...
- **Event Bridge System**: Bidirectional event mapping between Meltano and FLEXT enterprise systems
...
```mermaid
flowchart TD
    A[FLEXT Domain] --> B[Anti-Corruption Layer]
    B --> C[Meltano SDK]
    C --> D[Meltano Core]

    E[FLEXT Events] --> F[Event Bridge]
    F --> G[Meltano Events]
...
│   ├── FlextMeltanoProjectManager     # Enhanced project manager
...
│   ├── Automatic Mapping           # FLEXT ↔ Meltano event translation
...
**Purpose**: Bidirectional event bridge with automatic mapping between Meltano and FLEXT systems
...
class MeltanoEventBridge:
    ...
    def __init__(self, flext_event_bus: EventBus, meltano_event_system: Any):
        self.flext_event_bus = flext_event_bus
...
- **Enterprise Integration**: Built-in integration with FLEXT enterprise systems
...
```mermaid
graph TD
    A[FLEXT Meltano Layer] --> B[Meltano SDK]
    A --> C[Singer SDK]
    A --> D[PyArrow]

    E[FLEXT Domain] --> A
    F[FLEXT Application] --> A
    G[FLEXT Infrastructure] --> A
...
```mermaid
graph LR
    A[FLEXT Request] --> B[Anti-Corruption Layer]
    B --> C[Meltano SDK]
    C --> D[Singer Taps/Targets]
    D --> E[Data Sources/Destinations]

    F[FLEXT Events] --> G[Event Bridge]
    G --> H[Meltano Events]
...
from flext_core.meltano.unified_anti_corruption_layer import UnifiedMeltanoAntiCorruptionLayer
...
PYTHONPATH=src python -c "
from flext_core.meltano.project_manager import FlextMeltanoProjectManager
manager = FlextMeltanoProjectManager('/tmp/test-project')
print(f'✅ Project Manager: {type(manager).__name__}')
"

# ✅ VERIFIED: Extensions System
PYTHONPATH=src python -c "
from flext_core.meltano.extensions import get_all_extensions
