...
> **General FLEXT law & workspace conventions live in the root [`../AGENTS.md`](../AGENTS.md) — read it first.** It is the SSOT for facade layering (`c/t/p/m/u` + `r/e/x/h/d/s`), config/settings access, the `make`-only build/test workflow, the testing law, and multi-agent git discipline. This file adds ONLY `flext-meltano`-specific knowledge and never repeats the root.
>
...
Enterprise data-integration layer. Provides the **thin-driver Singer base classes** (ADR-006) that every `flext-tap-*`, `flext-target-*`, and `flext-dbt-*` connector extends. Connectors stay thin; all reusable pipeline/executor/consumer lifecycle lives here.

## Structure

```
src/flext_meltano/
├── api.py                 # FlextMeltano facade — .tap / .target / .dbt / .execute
├── cli.py                 # CLI entry (flext-meltano)
├── base.py                # FlextMeltanoServiceBase (superclass of service components)
├── pipeline_mgr.py        # FlextMeltanoPipelineManager — CLI pipeline orchestration boundary
├── services/
│   ├── consumer_bases/    # facade.py exposes Tap / Target / Dbt bases (connectors subclass these)
│   ├── declarative_tap.py # the ONE designated singer_sdk integration point
│   ├── executor_base.py   # FlextMeltanoExecutorBase — execution mechanics
│   ├── dbt_runner.py      # dbt invocation
│   ├── dbt_project.py     # dbt project model
│   ├── bridge.py          # adapter bridge
│   └── adapters.py
├── constants.py typings.py protocols.py models.py utilities.py   # AUTO-GENERATED facets
└── _constants/ _typings/ _protocols/ _models/ _utilities/         # private impl
```
...
|--------|------|----------|------|
| `FlextMeltano` | class | `api.py` | public facade: `.tap/.target/.dbt/.execute` |
| `FlextMeltanoServiceBase` | class | `base.py` | superclass of all service components |
| `Tap` / `Target` / `Dbt` | ABC | `services/consumer_bases/facade.py` | the bases connectors subclass |
| `FlextMeltanoTapServiceBase` | class | `services/consumer_bases/tap_service_base.py` | tap lifecycle |
| `FlextMeltanoExecutorBase` | class | `services/executor_base.py` | execution mechanics (bridge/abstractions import it) |
| `FlextMeltanoPipelineManager` | class | `pipeline_mgr.py` | called by `cli.py`; pipeline boundary |

## Conventions (specific to this package)

- Connectors extend `Tap` / `Target` / `Dbt` from `services/consumer_bases/facade.py` and keep connector-specific behavior + models in their own package (ADR-006 thin drivers).
- Service code round-trips owned payloads with `model_validate` / `model_dump` — keep typed-model boundaries, don't flatten to dicts.

## Anti-Patterns / Gotchas

- **Never `import singer_sdk` directly** in a connector — `services/declarative_tap.py` centralizes that import; use it.
- **Do not fork** executor/consumer lifecycle logic; extend the base classes.
- Downstream connectors get CLI-domain `u` helpers via `from flext_cli import u` (CLI domains are owned by `flext-cli`, not re-declared here).

## Commands

```bash
make check PROJECT=flext-meltano       # ruff/pyrefly/mypy/pyright
make test  PROJECT=flext-meltano       # tests/{unit,integration,e2e,extensions}
```
