...
""")
            ...
            if result.success:
                ...
                assert any(keyword in error_lower for keyword in [
                    "block", "plugin", "no such file", "meltano.yml"
                ])
...
""")
            ...
            result = bridge.run_pipeline_real(
                project_root,
                "tap-csv",
                "target-csv"
            )
            ...
            if result.success:
                ...
                assert result.value["execution_method"] == "meltano_singer_runner_native"
            else:
    ...
    def test_adapt_project_config_edge_cases(self) -> None:
        ...
        test_configs = [
            ...
            # Config com plugins como não-dict (edge case)
            {
                ...
                "plugins": "invalid-plugins-format"
            }
        ]
...
class TestMeltanoBridgeIntegrationAdvanced:
    ...
    def test_error_propagation_comprehensive(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result2 = bridge.execute_meltano_command_real(
                project_root,
                ["completely", "invalid", "command", "structure"]
            )
            assert result2.success is False
