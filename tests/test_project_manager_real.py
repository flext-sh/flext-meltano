...
class TestFlextMeltanoProjectInitializationMode:
    ...
    def test_initialization_mode_values(self) -> None:
        ...
        assert (
            FlextMeltanoProjectInitializationMode.FORCE_RECREATE.value
            == "force_recreate"
        )
        assert (
            FlextMeltanoProjectInitializationMode.OVERWRITE_EXISTING.value
            == "overwrite_existing"
        )
...
class TestFlextMeltanoProjectManager:
    ...
    def test_filter_singer_warnings(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
        ...
        filtered = project_manager._filter_singer_warnings(
            stderr_with_warnings,
        )
    ...
    def test_filter_singer_warnings_empty(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_project_config_success(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_project_config_not_found(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
        """Testa carregamento de configuração não encontrada."""
        result = await project_manager.load_project_config(
            "nonexistent-project",
        )
    ...
    @pytest.mark.asyncio
    async def test_save_project_config_success(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
        ...
        # Save config
        result = await project_manager.save_project_config(
            project_name,
            new_config,
        )
        assert result.success is True
    ...
    @pytest.mark.asyncio
    async def test_run_pipeline_direct_success(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_pipeline_direct_project_not_found(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_command_with_environment(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_add_plugin_add_failure(
        self,
        project_manager: FlextMeltanoProjectManager,
    ) -> None:
        ...
        with patch.object(project_manager, "run_command") as mock_run_command:
            mock_run_command.return_value = FlextResult.fail(
                "Add command failed",
            )
...
class TestFlextMeltanoFlextProjectManager:
    ...
    def test_enhanced_project_manager_with_event_bus(
        self,
        temp_project_root: Path,
        mock_event_bus: Mock,
    ) -> None:
        """Testa inicialização com event bus."""
        manager = FlextMeltanoFlextProjectManager(
            temp_project_root,
            mock_event_bus,
        )
    ...
    async def test_create_project_with_events_success(
        ...
        result = await enhanced_project_manager.create_project_with_events(
            "event-project",
        )
    ...
    async def test_backup_project_success(
        ...
        with patch("shutil.make_archive") as mock_make_archive:
            mock_make_archive.return_value = str(
                backup_path.with_suffix(".zip"),
            )

            result = await enhanced_project_manager.backup_project(
                project_name,
                backup_path,
            )
...
class TestProjectManagerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_enhanced_manager_inheritance(
        self,
        temp_project_root: Path,
    ) -> None:
        ...
        with patch("shutil.make_archive"):
            backup_result = await enhanced_manager.backup_project(
                "inheritance-test",
                backup_path,
            )
    ...
    def test_error_classes_inheritance(self) -> None:
        ...
        assert (
            isinstance(execution_error, FlextMeltanoProjectError) is False
