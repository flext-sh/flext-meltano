    ...
    def test_create_temp_directory_success(self) -> None:
        ...
        if result.is_success:
            ...
            self.test_assertions.assert_true(
                ...
                message="Should return Path",
            )
            self.test_assertions.assert_true(
                condition=temp_path.exists(),
                message="Temp directory should exist",
            )
    ...
    def test_create_meltano_config_dict_success(self) -> None:
        ...
        if result.is_success:
            ...
            self.test_assertions.assert_in(
                item="project_id",
                container=config,
                message="Should contain project_id",
            )
            self.test_assertions.assert_in(
                item="project_name",
                container=config,
            ...
            self.test_assertions.assert_in(
                item="plugins",
                container=config,
                message="Should contain plugins",
            )
            self.test_assertions.assert_in(
                item="metadata",
                container=config,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.is_success:
                ...
                self.test_assertions.assert_true(
                    ...
                    message="YAML file should be created",
                )
        ...
        if result.is_success:
            ...
            for field in expected_fields:
                self.test_assertions.assert_in(
                    item=field,
                    container=config,
                    message=f"Should contain {field}",
                )
