...
class TestFlextMeltanoUtilitiesComplete:
    ...
    def setup_method(self) -> None:
        ...
        self.functional_service = FlextTestsUtilities.functional_service("meltano_utilities")
    ...
    def test_create_temp_directory_success(self) -> None:
        ...
        result = FlextMeltanoUtilities.create_temp_directory(prefix="flext_test_")
        ...
        if result.success:
            ...
            self.test_assertions.assert_true(condition=isinstance(temp_path, Path), message="Should return Path")
            self.test_assertions.assert_true(condition=temp_path.exists(), message="Temp directory should exist")
            ...
            for dir_name in meltano_dirs:
                ...
                self.test_assertions.assert_true(condition=meltano_dir.exists(), message=f"{dir_name} should exist")
    ...
    def test_create_temp_directory_default_prefix(self) -> None:
        ...
        if result.success:
            ...
            self.test_assertions.assert_true(condition="flext_meltano_" in str(temp_path), message="Should contain default prefix")
    ...
    def test_create_meltano_config_dict_success(self) -> None:
        ...
        result = FlextMeltanoUtilities.create_meltano_config_dict(
            ...
            project_name="Test Meltano Project"
        )

        self.test_assertions.assert_true(condition=isinstance(result, FlextResult), message="Should return FlextResult")
        self.test_assertions.assert_true(condition=result.success, message="Config creation should succeed")

        if result.success:
            config = result.value
            self.test_assertions.assert_true(condition=isinstance(config, dict), message="Should return dict")

            # Validate config structure using flext_tests assertions
            self.test_assertions.assert_in(item="version", container=config, message="Should contain version")
            self.test_assertions.assert_in(item="project_id", container=config, message="Should contain project_id")
            self.test_assertions.assert_in(item="project_name", container=config, message="Should contain project_name")
            self.test_assertions.assert_in(item="environments", container=config, message="Should contain environments")
            self.test_assertions.assert_in(item="plugins", container=config, message="Should contain plugins")
            self.test_assertions.assert_in(item="metadata", container=config, message="Should contain metadata")

            # Validate specific values
            self.test_assertions.assert_equals(actual=config["version"], expected=1, message="Version should be 1")
            self.test_assertions.assert_equals(actual=config["project_id"], expected="test_project", message="Project ID should match")
    ...
    def test_create_meltano_config_dict_with_defaults(self) -> None:
        ...
        result = FlextMeltanoUtilities.create_meltano_config_dict(project_id="minimal_project")

        self.test_assertions.assert_true(condition=result.success, message="Config creation should succeed with defaults")
        ...
        if result.success:
            ...
            self.test_assertions.assert_equals(actual=config["project_name"], expected="", message="Should return empty string when project_name not provided")
    ...
    def test_create_meltano_config_dict_empty_inputs(self) -> None:
        ...
        self.test_assertions.assert_true(condition=result.success, message="Should handle empty project_id gracefully")

        if result.success:
            config = result.value
            # REAL BEHAVIOR: FlextUtilities.TextProcessor.safe_string returns empty string when input is empty
            self.test_assertions.assert_equals(actual=config["project_id"], expected="", message="Should return empty string when project_id is empty")
    ...
    def test_write_meltano_yml_success(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.test_assertions.assert_true(condition=isinstance(result, FlextResult), message="Should return FlextResult")
            self.test_assertions.assert_true(condition=result.success, message="YAML writing should succeed")

            if result.success:
                # Verify file was created and has content
                self.test_assertions.assert_true(condition=yaml_file.exists(), message="YAML file should be created")

                # Read and verify content
                content = yaml_file.read_text(encoding="utf-8")
                self.test_assertions.assert_in(item="version: 1", container=content, message="Should contain version")
                self.test_assertions.assert_in(item="test_yaml_project", container=content, message="Should contain project ID")
    ...
    def test_create_plugin_config_dict_success(self) -> None:
        ...
        self.test_assertions.assert_true(condition=isinstance(result, FlextResult), message="Should return FlextResult")
        self.test_assertions.assert_true(condition=result.success, message="Plugin config creation should succeed")
        ...
        if result.success:
            ...
            expected_fields = ["name", "namespace", "pip_url", "executable", "type", "settings", "config"]
            for field in expected_fields:
                self.test_assertions.assert_in(item=field, container=config, message=f"Should contain {field}")

            # Validate specific values
            self.test_assertions.assert_equals(actual=config["name"], expected="tap-csv", message="Name should match")
            self.test_assertions.assert_equals(actual=config["namespace"], expected="tap_csv", message="Namespace should match")
            self.test_assertions.assert_equals(actual=config["type"], expected="extractor", message="Type should match")
    ...
    def test_create_plugin_config_dict_with_defaults(self) -> None:
        ...
        self.test_assertions.assert_true(condition=result.success, message="Should succeed with defaults")

        if result.success:
            config = result.value

            # REAL BEHAVIOR - SOURCE OF TRUTH from actual execution:
            self.test_assertions.assert_equals(actual=config["type"], expected="extractor", message="Should default to extractor")
            self.test_assertions.assert_equals(actual=config["namespace"], expected="", message="Should return empty string when namespace not provided")
    ...
    def test_create_plugin_config_dict_empty_name(self) -> None:
        ...
        self.test_assertions.assert_true(condition=result.success, message="Should handle empty name gracefully")

        if result.success:
            config = result.value
            # REAL BEHAVIOR: FlextUtilities.TextProcessor.safe_string returns empty string, not fallback
            self.test_assertions.assert_equals(actual=config["name"], expected="", message="Should return empty string when name is empty")
    ...
    def test_complete_meltano_setup_workflow(self) -> None:
        ...
        temp_result = FlextMeltanoUtilities.create_temp_directory("workflow_test_")
        self.test_assertions.assert_true(condition=temp_result.success, message="Temp directory creation should succeed")
        ...
        if temp_result.success:
            ...
            self.test_assertions.assert_true(condition=config_result.success, message="Config creation should succeed")
            ...
            if config_result.success:
                ...
                write_result = FlextMeltanoUtilities.write_meltano_yml(config, meltano_yml_path)
                self.test_assertions.assert_true(condition=write_result.success, message="YAML writing should succeed")
                ...
                if write_result.success:
                    ...
                    self.test_assertions.assert_true(condition=plugin_result.success, message="Plugin config creation should succeed")
    ...
    def test_utilities_edge_cases(self) -> None:
        ...
        for project_id, project_name in edge_cases:
            result = FlextMeltanoUtilities.create_meltano_config_dict(project_id, project_name)
            self.test_assertions.assert_true(condition=result.success, message=f"Should handle edge case: {project_id}, {project_name}")
    ...
    def test_utilities_type_safety(self) -> None:
        ...
        temp_result = FlextMeltanoUtilities.create_temp_directory()
        self.test_assertions.assert_true(condition=isinstance(temp_result, FlextResult), message="create_temp_directory should return FlextResult")

        config_result = FlextMeltanoUtilities.create_meltano_config_dict("test")
        self.test_assertions.assert_true(condition=isinstance(config_result, FlextResult), message="create_meltano_config_dict should return FlextResult")

        plugin_result = FlextMeltanoUtilities.create_plugin_config_dict("test")
        self.test_assertions.assert_true(condition=isinstance(plugin_result, FlextResult), message="create_plugin_config_dict should return FlextResult")
