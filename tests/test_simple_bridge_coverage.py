...
class TestFlextMeltanoBridge:
    ...
    def test_bridge_initialization_with_config(self):
        ...
        config = FlextMeltanoConfig(project_root="/tmp/custom")
        bridge = FlextMeltanoBridge(config)

        assert bridge._config is not None
        assert bridge._config.project_root == "/tmp/custom"
    ...
    def test_get_version_success(self, mock_executor_class):
        ...
        assert result.data["python"] == f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    ...
    def test_get_version_failure(self, mock_executor_class):
        ...
        assert result.is_success  # Should still succeed with "unknown" version
    ...
    def test_list_plugins_failure(self, mock_executor_class):
        ...
        assert result.is_success  # Should return empty list on failure
        assert result.data == []
    ...
    def test_add_plugin_always_fails(self):
        ...
        assert not result.is_success
        assert "Plugin installation requires initialized Meltano project" in result.error
    ...
    def test_run_pipeline_with_environment(self, mock_executor_class):
        ...
        expected_cmd = ["run", "--environment", "prod", "tap-postgres", "target-warehouse"]
...
class TestCreateFlextMeltanoBridge:
    ...
    def test_create_bridge_with_config(self):
        ...
        config = FlextMeltanoConfig(
            project_root="/tmp/custom_project",
        ...
        assert bridge._config.project_root == "/tmp/custom_project"
...
class TestBridgeIntegration:
    ...
    def test_bridge_json_serialization(self):
        ...
        with patch('flext_meltano.simple_bridge.FlextMeltanoExecutor') as mock_executor_class:
            mock_executor = Mock()
            mock_executor_class.return_value = mock_executor
    ...
    def test_bridge_error_handling_patterns(self):
        ...
        assert not catalog_result.is_success
        assert not dbt_result.is_success
    ...
    def test_bridge_command_building(self, mock_executor_class):
        ...
        mock_executor.run_command.assert_called_with([
            "run", "--environment", "prod", "tap-postgres", "target-postgres"
        ])
